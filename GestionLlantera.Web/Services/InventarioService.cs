...
namespace GestionLlantera.Web.Services
{
    public class InventarioService : IInventarioService
    {
        ...
        /// <summary>
        /// Busca productos para facturación con paginación y filtros
        /// </summary>
        public async Task<List<ProductoDTO>> BuscarProductosAsync(string termino = "", int pagina = 1, int tamano = 20, string jwtToken = null)
        {
            try
            {
                _logger.LogInformation("🔍 Buscando productos para facturación. Término: '{Termino}', Página: {Pagina}, Tamaño: {Tamano}", 
                    termino, pagina, tamano);
                ...
                if (!string.IsNullOrEmpty(jwtToken))
                {
                    ...
                    _httpClient.DefaultRequestHeaders.Authorization =
                        new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", jwtToken);
                    _logger.LogInformation("🔐 Token JWT configurado para búsqueda de productos");
                }

                // ✅ CONSTRUIR URL CON PARÁMETROS DE BÚSQUEDA
                var url = "api/Inventario/productos/buscar";
                var parametros = new List<string>();

                if (!string.IsNullOrWhiteSpace(termino))
                {
                    parametros.Add($"termino={Uri.EscapeDataString(termino)}");
                }
                ...
                if (!response.IsSuccessStatusCode)
                {
                    _logger.LogError("❌ Error buscando productos: {StatusCode}", response.StatusCode);
                    
                    // ✅ FALLBACK: Si la API no tiene endpoint específico, usar el método existente y filtrar
                    var todosLosProductos = await ObtenerProductosAsync(jwtToken);
                    ...
                    _logger.LogInformation("✅ Fallback aplicado: {Count} productos encontrados", productosPaginados.Count);
                    return productosPaginados;
                }

                var content = await response.Content.ReadAsStringAsync();
                var productos = JsonConvert.DeserializeObject<List<ProductoDTO>>(content) ?? new List<ProductoDTO>();

                _logger.LogInformation("✅ Búsqueda exitosa: {Count} productos encontrados", productos.Count);
                return productos;
            }
            catch (Exception ex)
            {
