...
public class AuthController : ControllerBase
{
    ...
    #region Forzar regeneración por cambios de permisos
    /// <summary>
    /// Fuerza la regeneración del JWT cuando se detectan cambios en permisos de roles
    /// </summary>
    /// <param name="request">Lista de usuarios afectados por cambios de permisos</param>
    /// <returns>Resultado de la operación</returns>
    [HttpPost("forzar-regeneracion-jwt")]
    [AllowAnonymous]
    public async Task<IActionResult> ForzarRegeneracionJWT([FromBody] ForzarRegeneracionRequest request)
    {
        try
        {
            if (request?.UsuarioIds == null || !request.UsuarioIds.Any())
            {
                return BadRequest(new { message = "Lista de usuarios requerida" });
            }

            // Invalidar todas las sesiones de los usuarios afectados
            var sesionesAfectadas = await _context.SesionUsuario
                .Where(s => request.UsuarioIds.Contains(s.UsuarioId.Value) && s.EstaActiva == true)
                .ToListAsync();

            foreach (var sesion in sesionesAfectadas)
            {
                sesion.EstaActiva = false;
                sesion.FechaInvalidacion = DateTime.Now;
            }
            ...
            _logger?.LogInformation($"✅ Se invalidaron {sesionesAfectadas.Count} sesiones activas para {request.UsuarioIds.Count} usuarios por cambios en permisos");

            return Ok(new
            {
                message = "Sesiones invalidadas exitosamente",
                usuariosAfectados = request.UsuarioIds.Count,
                sesionesInvalidadas = sesionesAfectadas.Count
            });
        }
        catch (Exception ex)
        {
            _logger?.LogError(ex, "❌ Error al forzar regeneración de JWT");
            return StatusCode(500, new { message = "Error interno del servidor" });
        }
    }
    #endregion
...
/// <summary>
/// Request para forzar regeneración de JWT
/// </summary>
public class ForzarRegeneracionRequest
{
