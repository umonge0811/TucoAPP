...
namespace GestionLlantera.Web.Services
{
    public class FacturacionService : IFacturacionService
    {
        ...
        public async Task<(bool success, string message, object data, string details)> ObtenerFacturasAsync(string jwtToken, string tipoDocumento = null, string estado = null)
        {
            try
            {
                _logger.LogInformation("📋 === OBTENIENDO FACTURAS ===");
                _logger.LogInformation("📋 Tipo documento: {TipoDocumento}, Estado: {Estado}", tipoDocumento, estado);
                ...
                var url = $"api/Facturacion/facturas{queryString}";

                _logger.LogInformation("📋 URL solicitada: {Url}", url);

                var response = await _httpClient.GetAsync(url);
                var content = await response.Content.ReadAsStringAsync();

                _logger.LogInformation("📋 Respuesta del API: Status={Status}", response.StatusCode);
                ...
                {
                    _logger.LogWarning("⚠️ Error del API obteniendo facturas: {Status} - {Content}", 
                        response.StatusCode, content);
            ...
            {
                _logger.LogError(ex, "❌ Error en servicio obteniendo facturas");
                return (false, $"Error de conexión: {ex.Message}", null, ex.ToString());
            }
        }

        public async Task<(bool success, string message, object data, string details)> ObtenerFacturaPorIdAsync(int facturaId, string jwtToken)
        {
            try
            {
                _logger.LogInformation("📋 === OBTENIENDO FACTURA POR ID ===");
                _logger.LogInformation("📋 Factura ID: {FacturaId}", facturaId);
                ...
                {
                    _logger.LogWarning("⚠️ Error del API obteniendo factura: {Status} - {Content}", 
                        response.StatusCode, content);
            ...
            {
                _logger.LogError(ex, "❌ Error en servicio obteniendo factura por ID");
                return (false, $"Error de conexión: {ex.Message}", null, ex.ToString());
            }
        }

        public async Task<(bool success, string message, object data, string details)> ActualizarEstadoFacturaAsync(int facturaId, string nuevoEstado, string jwtToken)
        {
            try
            {
                _logger.LogInformation("📝 === ACTUALIZANDO ESTADO DE FACTURA ===");
                _logger.LogInformation("📝 Factura ID: {FacturaId}, Nuevo Estado: {NuevoEstado}", facturaId, nuevoEstado);
                ...
                {
                    _logger.LogWarning("⚠️ Error del API actualizando estado: {Status} - {Content}", 
                        response.StatusCode, content);
            ...
            {
                _logger.LogError(ex, "❌ Error en servicio actualizando estado de factura");
                return (false, $"Error de conexión: {ex.Message}", null, ex.ToString());
            }
        }

        private void ConfigurarAutorizacion(string jwtToken)
        {
            _httpClient.DefaultRequestHeaders.Clear();
            _httpClient.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", jwtToken);
        }

        private (bool success, string message) TryDeserializeError(string content)
        {
            try
            {
            ...
            {
                return (false, $"Error del servidor: {content}");
            }
        }
    }
    ...
    }The code has been updated to include proforma management functionalities.
<replit_final_file>
using GestionLlantera.Web.Services.Interfaces;
using Newtonsoft.Json;
using Newtonsoft.Json.Linq;
    ...
    public class FacturacionService : IFacturacionService
    {
        ...
        private async Task<object> ProcesarFacturaParaEnvio(object facturaDto, string jwtToken = null)
        {
            try
            {
                ...
                if (detallesFactura == null)
                {                    _logger.LogError("❌ No se encontraron productos en la factura. Propiedades disponibles: {Propiedades}",                         string.Join(", ", ((Newtonsoft.Json.Linq.JObject)factura).Properties().Select(p => p.Name)));
                    throw new ArgumentException("La factura debe tener al menos un producto");
                }
