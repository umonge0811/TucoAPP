...
async function inicializarEjecutorInventario(inventarioId) {
    try {
        ...
        // ✅ VERIFICAR SI VIENE DE NOTIFICACIÓN DE RECONTEO
        verificarParametroReconteo();
...
/**
 * ✅ FUNCIÓN: Solicitar reconteo de un producto específico
 */
async function solicitarReconteoProducto(inventarioId, productoId, nombreProducto) {
    try {
        console.log('🔄 Solicitando reconteo para producto:', productoId);

        // Obtener usuarios asignados al inventario
        const usuariosAsignados = await obtenerUsuariosAsignados(inventarioId);

        // Crear opciones de usuarios
        let opcionesUsuarios = '';
        if (usuariosAsignados && usuariosAsignados.length > 0) {
            opcionesUsuarios = usuariosAsignados.map(usuario => 
        ...
        const { value: formValues } = await Swal.fire({
            ...
            html: `
                <div class="text-start">
                    ...
                    <div class="mb-3">
                        <label class="form-label">Asignar reconteo a:</label>
                        <select id="usuarioReconteo" class="form-select">
                            <option value="">Seleccionar usuario...</option>
                            ${opcionesUsuarios}
                        </select>
                        <small class="text-muted">Si no seleccionas usuario, se notificará al contador original</small>
                    </div>
                    
                    <div class="mb-3">
                        <label class="form-label">Motivo del reconteo:</label>
                        <select id="motivoReconteo" class="form-select">
                            <option value="Discrepancia detectada">Discrepancia detectada</option>
                            <option value="Verificación de calidad">Verificación de calidad</option>
                            <option value="Producto no ubicado">Producto no ubicado</option>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea id="observacionesReconteo" class="form-control" rows="3" 
                                  placeholder="Detalles adicionales sobre por qué se necesita el reconteo..."></textarea>
                    </div>
                </div>
            `,
            showCancelButton: true,
            ...
            width: '500px',
            preConfirm: () => {
                const usuarioId = document.getElementById('usuarioReconteo').value;
                const motivo = document.getElementById('motivoReconteo').value;
                const observaciones = document.getElementById('observacionesReconteo').value;
                ...
                return {
                    usuarioAsignadoId: usuarioId ? parseInt(usuarioId) : null,
                    motivo: motivo,
        ...
        // Enviar solicitud de reconteo
        const response = await fetch(`/api/TomaInventario/${inventarioId}/productos/${productoId}/solicitar-reconteo`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
        ...
        if (response.ok && result.success) {
            await Swal.fire({
                ...
                html: `
                    <div class="text-center">
                        <p>La solicitud de reconteo ha sido enviada exitosamente.</p>
                        <div class="alert alert-success mt-3">
                            <strong>Notificado a:</strong> ${result.usuarioNotificado}<br>
                            <strong>Producto:</strong> ${result.productoNombre}
                        </div>
                    </div>
                `,
                confirmButtonText: 'Entendido'
            ...
            // Actualizar la UI para mostrar que se solicitó reconteo
            marcarProductoParaReconteo(productoId);
        ...
        console.error('❌ Error solicitando reconteo:', error);
        
        await Swal.fire({
            icon: 'error',
            title: 'Error',
            text: `No se pudo enviar la solicitud de reconteo: ${error.message}`,
            confirmButtonText: 'Entendido'
        });
    }
}
...
/**
 * ✅ FUNCIÓN: Obtener usuarios asignados al inventario
 */
async function obtenerUsuariosAsignados(inventarioId) {
    try {
        ...
        if (response.ok) {
            ...
            return inventario.asignacionesUsuarios || [];
        }
        return [];
    } catch (error) {
...
/**
 * ✅ FUNCIÓN: Marcar producto para reconteo en la UI
 */
function marcarProductoParaReconteo(productoId) {
    const fila = $(`tr[data-producto-id="${productoId}"]`);
    if (fila.length) {
        // Agregar badge de reconteo solicitado
        const estadoCell = fila.find('.estado-conteo');
        if (estadoCell.length) {
            estadoCell.append(' <span class="badge bg-warning">Reconteo Solicitado</span>');
        }
...
function verificarParametroReconteo() {
    try {
        ...
        const productoRecontar = urlParams.get('recontar');
        
        if (productoRecontar) {
            console.log('🔄 Producto para recontar detectado:', productoRecontar);
            
            // Esperar un momento para que la tabla se cargue
            setTimeout(() => {
                resaltarProductoParaReconteo(productoRecontar);
                mostrarAlertaReconteo(productoRecontar);
            }, 2000);
        }
    } catch (error) {
        console.error('❌ Error verificando parámetro de reconteo:', error);
    }
}

/**
 * ✅ FUNCIÓN: Resaltar producto que necesita reconteo
 */
function resaltarProductoParaReconteo(productoId) {
    try {
        const fila = $(`tr[data-producto-id="${productoId}"]`);
        if (fila.length) {
            // Resaltar la fila
            fila.addClass('table-warning').addClass('destacar-reconteo');
            
            // Agregar badge de reconteo pendiente
            const estadoCell = fila.find('.estado-conteo');
            if (estadoCell.length && !estadoCell.find('.badge-reconteo').length) {
                estadoCell.prepend('<span class="badge bg-info badge-reconteo me-2">📋 Reconteo Solicitado</span>');
            }
            ...
            // Efecto de pulsación
            fila.effect("pulsate", { times: 3 }, 2000);
            
            console.log('✅ Producto resaltado para reconteo:', productoId);
        } else {
            console.warn('⚠️ Fila del producto no encontrada:', productoId);
        }
    } catch (error) {
        console.error('❌ Error resaltando producto:', error);
    }
}

/**
 * ✅ FUNCIÓN: Mostrar alerta de reconteo solicitado
 */
function mostrarAlertaReconteo(productoId) {
    try {
        const producto = productosInventario.find(p => p.productoId == productoId);
        const nombreProducto = producto ? producto.nombreProducto : `Producto ID: ${productoId}`;
        ...
        Swal.fire({
            ...
            html: `
                <div class="text-center">
                    <p>Te han solicitado recontar el siguiente producto:</p>
                    <div class="alert alert-info mt-3">
                        <strong>${nombreProducto}</strong>
                    </div>
                    <p class="text-muted">El producto está resaltado en la tabla para facilitar su ubicación.</p>
                </div>
            `,
            confirmButtonText: '📋 Entendido, procederé al reconteo',
            allowOutsideClick: false
        }).then(() => {
            // Limpiar parámetro de URL sin recargar
            const url = new URL(window.location);
            url.searchParams.delete('recontar');
            window.history.replaceState({}, document.title, url);
        });
        
    } catch (error) {
        console.error('❌ Error mostrando alerta de reconteo:', error);
    }
}
...
window.solicitarReconteoProducto = solicitarReconteoProducto;
