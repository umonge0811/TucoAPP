...
/**
 * ✅ VERIFICAR STOCK DE PRODUCTOS EN FACTURA PENDIENTE
 */
async function verificarStockProductosFactura() {
    try {
        console.log('🔍 === VERIFICANDO STOCK DE PRODUCTOS ===');
        console.log('🔍 Productos a verificar:', productosEnVenta.length);

        if (productosEnVenta.length === 0) {
            return { stockSuficiente: true, productosConProblemas: [] };
        }

        // Preparar lista de productos para verificar
        const productosVerificar = productosEnVenta.map(producto => ({
            productoId: producto.productoId,
            nombreProducto: producto.nombreProducto,
            cantidadRequerida: producto.cantidad
        }));

        console.log('📦 Productos a verificar:', productosVerificar);

        // Llamar al endpoint de verificación de stock
        const response = await fetch('/Facturacion/VerificarStock', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-Requested-With': 'XMLHttpRequest'
            },
            body: JSON.stringify(productosVerificar),
            credentials: 'include'
        });

        if (!response.ok) {
            console.warn('⚠️ No se pudo verificar stock, continuando con el proceso');
            return { stockSuficiente: true, productosConProblemas: [] };
        }

        const resultado = await response.json();
        console.log('📋 Resultado verificación stock:', resultado);

        if (resultado.success) {
            const productosConProblemas = resultado.data?.productosConProblemas || [];
            const stockSuficiente = productosConProblemas.length === 0;

            console.log('✅ Verificación completada:', {
                stockSuficiente,
                problemasEncontrados: productosConProblemas.length
            });
            ...
            console.warn('⚠️ Error en verificación de stock, continuando');
            return { stockSuficiente: true, productosConProblemas: [] };
        }
        ...
        console.warn('⚠️ Error verificando stock, continuando con precaución');
        return { stockSuficiente: true, productosConProblemas: [] };
    }
}

/**
 * ✅ MOSTRAR ERROR DE STOCK INSUFICIENTE CON OPCIONES
 */
async function mostrarErrorStockInsuficiente(productosConProblemas) {
    console.log('⚠️ === MOSTRANDO ERROR DE STOCK INSUFICIENTE ===');
    console.log('⚠️ Productos con problemas:', productosConProblemas);

    // Construir mensaje detallado
    let mensajeDetallado = '<div class="alert alert-warning mb-3">';
    mensajeDetallado += '<h6><i class="bi bi-exclamation-triangle me-2"></i>Los siguientes productos no tienen suficiente stock:</h6>';
    mensajeDetallado += '<ul class="mb-0">';
    ...
    productosConProblemas.forEach(problema => {
        mensajeDetallado += `
            <li>
                <strong>${problema.nombreProducto}</strong><br>
                <small class="text-muted">
                    Requerido: ${problema.cantidadRequerida} | 
                    Disponible: ${problema.stockDisponible} | 
                    Faltante: ${problema.cantidadRequerida - problema.stockDisponible}
                </small>
            </li>
        `;
    });
    ...
    mensajeDetallado += '<div class="alert alert-info">';
    mensajeDetallado += '<h6><i class="bi bi-lightbulb me-2"></i>¿Qué deseas hacer?</h6>';
    mensajeDetallado += '<p class="mb-0">Puedes ajustar las cantidades o remover los productos sin stock.</p>';
    ...
    const resultado = await Swal.fire({
        ...
        showCancelButton: true,
        showDenyButton: true,
        confirmButtonText: '<i class="bi bi-pencil me-1"></i>Ajustar Cantidades',
        denyButtonText: '<i class="bi bi-trash me-1"></i>Remover Productos',
        cancelButtonText: '<i class="bi bi-x-circle me-1"></i>Cancelar',
    ...
    if (resultado.isConfirmed) {
        ...
        // Remover productos sin stock
        removerProductosSinStock(productosConProblemas);
    }
 ...
 * ✅ AJUSTAR CANTIDADES AUTOMÁTICAMENTE
 */
function ajustarCantidadesProductosAutomaticamente(productosConProblemas) {
    console.log('🔧 === AJUSTANDO CANTIDADES AUTOMÁTICAMENTE ===');

    let productosAjustados = 0;
    let productosRemovidos = 0;

    productosConProblemas.forEach(problema => {
        const indiceProducto = productosEnVenta.findIndex(p => p.productoId === problema.productoId);
        
        if (indiceProducto !== -1) {
            if (problema.stockDisponible > 0) {
                // Ajustar cantidad al stock disponible
                productosEnVenta[indiceProducto].cantidad = problema.stockDisponible;
                productosEnVenta[indiceProducto].stockDisponible = problema.stockDisponible;
                productosAjustados++;
                console.log(`✅ Ajustado: ${problema.nombreProducto} -> ${problema.stockDisponible}`);
            } else {
                // Remover producto si no hay stock
                productosEnVenta.splice(indiceProducto, 1);
                productosRemovidos++;
                console.log(`❌ Removido: ${problema.nombreProducto} (sin stock)`);
            }
        }
    });
    ...
    if (productosAjustados > 0) {
        mensaje += `✅ ${productosAjustados} producto(s) ajustado(s) al stock disponible<br>`;
    }
    if (productosRemovidos > 0) {
        mensaje += `❌ ${productosRemovidos} producto(s) removido(s) (sin stock)`;
    }
    ...
    console.log('✅ Ajustes completados:', { productosAjustados, productosRemovidos });
}

/**
 * ✅ REMOVER PRODUCTOS SIN STOCK
 */
function removerProductosSinStock(productosConProblemas) {
    console.log('🗑️ === REMOVIENDO PRODUCTOS SIN STOCK ===');

    let productosRemovidos = 0;

    // Remover productos sin stock suficiente
    productosConProblemas.forEach(problema => {
        const indiceProducto = productosEnVenta.findIndex(p => p.productoId === problema.productoId);
        ...
        if (indiceProducto !== -1) {
            productosEnVenta.splice(indiceProducto, 1);
            productosRemovidos++;
            console.log(`❌ Removido: ${problema.nombreProducto}`);
        }
    });
    ...
    if (productosEnVenta.length === 0) {
        Swal.fire({
            title: 'Carrito Vacío',
            text: 'Se removieron todos los productos. La factura no puede completarse.',
            icon: 'warning',
            confirmButtonText: 'Entendido'
        });
    } else {
        Swal.fire({
            ...
            html: `❌ ${productosRemovidos} producto(s) removido(s)<br>✅ ${productosEnVenta.length} producto(s) restante(s)`,
            icon: 'info',
            confirmButtonText: 'Continuar',
            timer: 3000,
            timerProgressBar: true
    ...
    console.log('✅ Remoción completada:', { productosRemovidos, productosRestantes: productosEnVenta.length });
}
...
window.verificarStockProductosFactura = verificarStockProductosFactura;
window.mostrarErrorStockInsuficiente = mostrarErrorStockInsuficiente;
window.ajustarCantidadesProductosAutomaticamente = ajustarCantidadesProductosAutomaticamente;
window.removerProductosSinStock = removerProductosSinStock;
