    ...
    public class FacturacionController : Controller
    {
        ...
        public async Task<IActionResult> ObtenerProformas()
        {
            try
            {
                ...
                // Usar el servicio de facturación para obtener solo proformas
                var resultado = await _facturacionService.ObtenerFacturasAsync(jwtToken, tipoDocumento: "Proforma");
        ...
        public async Task<IActionResult> ObtenerFactura(int id)
        {
            try
            {
                _logger.LogInformation("📋 === OBTENIENDO FACTURA/PROFORMA ID: {Id} ===", id);
                ...
                var resultado = await _facturacionService.ObtenerFacturaPorIdAsync(id, jwtToken);
                ...
                else
                {
                    return Json(new { 
                        success = false, 
                        message = resultado.message ?? "Factura no encontrada"
                    });
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "❌ Error obteniendo factura: {Id}", id);
                return Json(new { 
                    success = false, 
                    message = "Error interno del servidor"
                });
        ...
        [HttpPost]
        public async Task<IActionResult> ConvertirProformaAFactura([FromBody] ConvertirProformaRequest request)
        {
            try
            {
                _logger.LogInformation("🔄 === CONVIRTIENDO PROFORMA A FACTURA ===");
                ...
                var proforma = proformaOriginal.data;

                // Crear nueva factura basada en la proforma
                var facturaData = new FacturaDTO
                {
                    ClienteId = GetProperty<int?>(proforma, "clienteId", null),
                    NombreCliente = GetProperty<string>(proforma, "nombreCliente", "Cliente General"),
                    IdentificacionCliente = GetProperty<string>(proforma, "identificacionCliente", ""),
                    TelefonoCliente = GetProperty<string>(proforma, "telefonoCliente", ""),
                    EmailCliente = GetProperty<string>(proforma, "emailCliente", ""),
                    DireccionCliente = GetProperty<string>(proforma, "direccionCliente", ""),
                    FechaFactura = DateTime.Now,
                    FechaVencimiento = null,
                    Subtotal = GetProperty<decimal>(proforma, "subtotal", 0),
                    DescuentoGeneral = GetProperty<decimal?>(proforma, "descuentoGeneral", null),
                    PorcentajeImpuesto = GetProperty<decimal?>(proforma, "porcentajeImpuesto", 13),
                    MontoImpuesto = GetProperty<decimal?>(proforma, "montoImpuesto", null),
                    Total = GetProperty<decimal>(proforma, "total", 0),
                    Estado = request.Estado ?? "Pendiente",
                    TipoDocumento = "Factura", // ✅ Cambiar de Proforma a Factura
                    MetodoPago = request.MetodoPago,
                    Observaciones = $"Factura generada desde proforma {GetProperty<string>(proforma, "numeroFactura", "N/A")}",
                    UsuarioCreadorId = ObtenerInfoUsuario().usuarioId,
                    DetallesPago = request.DetallesPago ?? new List<DetallePagoWebDTO>(),
                    DetallesFactura = ExtractDetallesFactura(proforma)
                };
                ...
                if (resultado.success)
                {
                    // Marcar la proforma original como convertida o anular
                    await MarcarProformaComoConvertida(request.ProformaId, jwtToken);
                    ...
                    return Json(new
                    {
                        ...
                        data = resultado.data,
                        numeroFactura = ExtractNumeroFactura(resultado.data),
                        facturaId = ExtractFacturaId(resultado.data)
                    });
                }
                else
                {
                    ...
                    {
                        success = false,
                        message = resultado.message ?? "Error al convertir proforma",
                        details = resultado.details
                    });
                }
            }
            ...
            {
                _logger.LogError(ex, "❌ Error convirtiendo proforma a factura: {ProformaId}", request?.ProformaId);
                return Json(new
                {
                    success = false,
                    message = "Error interno del servidor: " + ex.Message
        ...
        private List<DetalleFacturaDTO> ExtractDetallesFactura(object proforma)
        {
            try
            {
                var detallesProperty = GetProperty<object>(proforma, "detallesFactura", null);
                if (detallesProperty == null) return new List<DetalleFacturaDTO>();

                var json = System.Text.Json.JsonSerializer.Serialize(detallesProperty);
                var detalles = System.Text.Json.JsonSerializer.Deserialize<List<Dictionary<string, object>>>(json);

                return detalles?.Select(d => new DetalleFacturaDTO
                {
                    ProductoId = GetDictionaryValue<int>(d, "productoId", 0),
                    NombreProducto = GetDictionaryValue<string>(d, "nombreProducto", ""),
                    DescripcionProducto = GetDictionaryValue<string>(d, "descripcionProducto", ""),
                    Cantidad = GetDictionaryValue<int>(d, "cantidad", 1),
                    PrecioUnitario = GetDictionaryValue<decimal>(d, "precioUnitario", 0),
                    PorcentajeDescuento = GetDictionaryValue<decimal?>(d, "porcentajeDescuento", null),
                    MontoDescuento = GetDictionaryValue<decimal?>(d, "montoDescuento", null),
                    Subtotal = GetDictionaryValue<decimal>(d, "subtotal", 0)
                }).ToList() ?? new List<DetalleFacturaDTO>();
            }
            catch (Exception ex)
            {
                _logger.LogWarning("⚠️ Error extrayendo detalles de factura: {Error}", ex.Message);
                return new List<DetalleFacturaDTO>();
            }
        }

        private T GetDictionaryValue<T>(Dictionary<string, object> dict, string key, T defaultValue)
        {
            try
            {
            ...
            {
                _logger.LogWarning("⚠️ Error convirtiendo valor del diccionario {Key}: {Error}", key, ex.Message);
            }
            return defaultValue;
        }

        private string ExtractNumeroFactura(object data)
        {
            try
            {
                return GetProperty<string>(data, "numeroFactura", "N/A");
            }
            catch
            {
                return "N/A";
        ...
        private int? ExtractFacturaId(object data)
        {
            try
            {
                return GetProperty<int?>(data, "facturaId", null);
            }
            catch
            {
                return null;
        ...
        private async Task MarcarProformaComoConvertida(int proformaId, string jwtToken)
        {
            try
            {
                // Actualizar el estado de la proforma a "Convertida" o similar
                // Esto se puede hacer a través del endpoint de actualizar estado
                await _facturacionService.ActualizarEstadoFacturaAsync(proformaId, "Convertida", jwtToken);
            }
            catch (Exception ex)
            {
                _logger.LogWarning("⚠️ No se pudo marcar la proforma como convertida: {Error}", ex.Message);
            }
        }
    }
