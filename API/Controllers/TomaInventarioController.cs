...
namespace API.Controllers
{
    ...
    public class TomaInventarioController : ControllerBase
    {
        ...
        /// <summary>
        /// Solicita el reconteo de un producto específico y notifica al usuario asignado
        /// </summary>
        ...
        public async Task<IActionResult> SolicitarReconteoProducto(int inventarioId, int productoId, [FromBody] SolicitudReconteoDTO solicitud)
        {
            try
            {
                _logger.LogInformation("🔄 === SOLICITANDO RECONTEO DE PRODUCTO ===");
                ...
                // Obtener ID del usuario actual (quien solicita el reconteo)
                var usuarioSolicitanteId = ObtenerIdUsuarioActual();
                ...
                {
                    return NotFound(new { success = false, message = "Producto no encontrado en este inventario" });
                }
                ...
                int usuarioANotificar;
                
                if (solicitud.UsuarioAsignadoId.HasValue)
                {
                    // Notificar al usuario específico seleccionado
                    usuarioANotificar = solicitud.UsuarioAsignadoId.Value;
                }
                else
                {
                    // Notificar al usuario que hizo el conteo original, o al creador del inventario
                    usuarioANotificar = detalleInventario.UsuarioConteoId ?? inventario.UsuarioCreadorId;
                }

                // Obtener información del usuario a notificar
                var usuarioDestino = await _context.Usuarios
                    .FirstOrDefaultAsync(u => u.UsuarioId == usuarioANotificar);
                ...
                var urlAccion = $"/TomaInventario/Ejecutar/{inventarioId}?recontar={productoId}";

                await _notificacionService.CrearNotificacionAsync(
                    usuarioId: usuarioANotificar,
                    titulo: titulo,
                    mensaje: mensaje,
                    tipo: "warning",
                    icono: "fas fa-redo",
                    urlAccion: urlAccion,
                    entidadTipo: "ReconteoProducto",
            ...
            {
                _logger.LogError(ex, "❌ Error solicitando reconteo para producto {ProductoId}", productoId);
                return StatusCode(500, new { success = false, message = "Error interno del servidor" });
            }
        }
