document.addEventListener('DOMContentLoaded', function () {
    ...
    // DIAGN�STICO - Verificar elementos de utilidad
    console.log('=== DIAGN�STICO DE ELEMENTOS DE UTILIDAD ===');
    ...
    // Funci�n para normalizar texto a MAY�SCULAS
    function normalizarAMayusculas(texto) {
        if (!texto || typeof texto !== 'string') return '';

        // Solo convertir a may�sculas, mantener espacios internos
        // Solo eliminar espacios al inicio y final cuando sea el evento blur, no en tiempo real
        return texto.toUpperCase();
    }

    // Funci�n para normalizar texto a MAY�SCULAS (alias para compatibilidad)
    ...
    // Funci�n para aplicar normalizaci�n a un input
    function aplicarNormalizacionAInput(input, tipoNormalizacion = 'mayuscula') {
        ...
        const aplicarNormalizacion = () => {
            ...
            if (valorActual !== valorNormalizado) {
                ...
                console.log(`?? Texto normalizado a MAY�SCULAS: "${valorActual}" ? "${valorNormalizado}"`);
            }
        };

        // Funci�n para aplicar normalizaci�n final (con trim)
        const aplicarNormalizacionFinal = () => {
            ...
            if (valorActual !== valorNormalizado) {
                input.value = valorNormalizado;
                console.log(`?? Texto normalizado final: "${valorActual}" ? "${valorNormalizado}"`);
            }
        };

        // Aplicar normalizaci�n final al salir del campo (blur) - con trim
        ...
        // Normalizaci�n en tiempo real - solo may�sculas, SIN trim
        input.addEventListener('input', function () {
            const valorActual = this.value;
            const valorNormalizado = valorActual.toUpperCase(); // Solo may�sculas, mantener espacios
    ...
    // Funci�n para configurar normalizaci�n en todos los inputs de texto relevantes
    function configurarNormalizacionInputs() {
        console.log('?? Configurando normalizaci�n MAY�SCULAS en todos los inputs...');

        // TODOS los inputs de texto se normalizar�n a MAY�SCULAS
        ...
        // Aplicar normalizaci�n a MAY�SCULAS a todos los inputs de texto
        todosLosInputsTexto.forEach(selector => {
            const input = document.querySelector(selector);
            if (input) {
                aplicarNormalizacionAInput(input, 'mayuscula');
                console.log(`? Normalizaci�n MAY�SCULAS configurada para: ${selector}`);
            }
        });

        console.log('? Normalizaci�n MAY�SCULAS configurada en todos los inputs de texto');
    }
    ...
    function esCampoVisible(campo) {
        ...
        // ? NUEVO: Si estamos en modo llanta, ignorar TODOS los campos de informaci�n b�sica
        if (esLlantaCheckbox && esLlantaCheckbox.checked && informacionBasica && informacionBasica.contains(campo)) {
            console.log(`? Campo ${campo.name || campo.id} en Informaci�n B�sica - ignorado en modo llanta`);
            return false;
        }
        ...
        console.log(`? Campo ${campo.name || campo.id} - validaci�n activa`);
        return true;
    }
    ...
    // ========================================
    // GESTI�N DE TIPO DE PRODUCTO (LLANTA)
    // ========================================

    // NUEVA FUNCI�N: Sincronizar campos de llanta a campos principales
    // FUNCI�N MEJORADA: Sincronizar campos de llanta
    function sincronizarCamposLlanta() {
        console.log('?? Configurando sincronizaci�n de campos de llanta...');

        // ? NO sincronizar nombre - se generar� autom�ticamente
        ...
        console.log('? Sincronizaci�n configurada (sin nombre del producto)');
    }
    ...
    function prepararFormularioParaEnvio() {
        ...
        if (esLlantaCheckbox && esLlantaCheckbox.checked) {
            ...
            // ? LIMPIAR CAMPOS DE INFORMACI�N B�SICA PARA QUE NO INTERFIERAN
            ...
            const esMoto = document.getElementById('esMotoCheckbox')?.checked || false;

            let nombreGenerado = 'Llanta';
            // Agregar MOTO si el checkbox está marcado
            if (esMoto) {
                nombreGenerado += ' MOTO';
            }
            ...
            if (descripcionBasica) {
                ...
                console.log(`? Descripci�n establecida: "${valorDescripcion}"`);
            }
    ...
    // FUNCI�N MEJORADA: Limpiar campos de llanta
    ...
    if (esLlantaCheckbox && llantaFields && tipoProductoInfo && textoTipoProducto) {
        // AGREGAR ESTA L�NEA - Referencia al card de informaci�n b�sica
        const informacionBasica = document.getElementById('informacionBasica');
        ...
        function actualizarTipoProducto() {
            if (esLlantaCheckbox.checked) {
                ...
                console.log('? Modo producto general activado - Validaciones restauradas en informaci�n b�sica');
            }
        }
...
`;
        ...
        // ========================================
        // BOT�N PARA LIMPIAR CAMPOS DE PRECIO
        // ========================================
        ...
        if (btnLimpiarPrecios) {
            ...
            console.log('? Bot�n limpiar precios configurado');
        }
    ...
    if (form && submitButton) {
        ...
        function validarFormularioCompleto() {
            ...
            formInputs.forEach(input => {
                ...
                if (input.hasAttribute('required') && esCampoVisible(input)) {
                    if (!validarCampo(input)) {
                        ...
                        console.log(`? Campo v�lido: ${input.name || input.id}`);
                    }
                }
            });
            ...
            if (modoAutomaticoRadio && modoAutomaticoRadio.checked) {
                ...
                if (!inputPrecioManual || !inputPrecioManual.value || parseFloat(inputPrecioManual.value) <= 0) {
                    ...
                    esValido = false;
                    console.log(`? Precio manual inv�lido`);
                }
            }

            console.log(`Validaci�n completa: ${esValido ? 'V�LIDO' : 'INV�LIDO'}`);
            return esValido;
        }
        ...
        form.onsubmit = function (e) {
            ...
            // ? NUEVO: Preparar una vez m�s justo antes del env�o (por seguridad)
            prepararFormularioParaEnvio();
    ...
    function configurarAutocompletadoMarca() {
        ...
        marcaInput.addEventListener('input', function () {
            ...
            console.log(`?? Usuario escribi�: "${valor}"`);
    ...
    function seleccionarMarca(marca, input, container, esNueva) {
        ...
        // Normalizar la marca a MAY�SCULAS antes de asignarla
        ...
        if (esNueva && typeof toastr !== 'undefined') {
            toastr.info(`Nueva marca "${marcaNormalizada}" ser� agregada al guardar el producto`);
        }
    ...
    function configurarAutocompletadoModelo() {
        ...
        if (!modeloInput || !modeloSuggestions) {
            console.warn('?? Elementos de autocompletado para modelo no encontrados');
            return;
        }
        ...
        modeloInput.addEventListener('input', function () {
            ...
            console.log(`?? Usuario escribi� modelo: "${valor}", marca actual: "${marcaSeleccionada}"`);
        ...
        if (marcaInput) {
            marcaInput.addEventListener('change', function () {
                console.log('?? Marca cambi�, limpiando modelo');
                modeloInput.value = '';
                ocultarSugerencias(modeloSuggestions);
            });
        }
    ...
    // ========================================
    // AUTOCOMPLETADO PARA �NDICE DE VELOCIDAD
    // ========================================
    ...
    function configurarAutocompletadoIndiceVelocidad() {
        ...
        if (!input || !suggestions) {
            console.warn('?? Elementos de autocompletado para �ndice de velocidad no encontrados');
            return;
        }

        console.log('? Configurando autocompletado para �ndice de velocidad');
    ...
    function configurarAutocompletadoTipoTerreno() {
        ...
        if (!input || !suggestions) {
            console.warn('?? Elementos de autocompletado para tipo de terreno no encontrados');
            return;
        }
    ...
    // ========================================
    // FUNCI�N GEN�RICA PARA AUTOCOMPLETADO
    // ========================================
    ...
    function configurarAutocompletadoGenerico(input, suggestions, nombreCampo, url) {
        ...
        input.addEventListener('input', function () {
            const valor = this.value.trim();
            console.log(`?? Usuario escribi� ${nombreCampo}: "${valor}"`);
            ...
            // Si es muy corto, esperar m�s caracteres
            if (valor.length < 1) {
                mostrarMensaje(suggestions, '?? Escriba al menos 1 car�cter...');
                return;
            }
    ...
    // ========================================
    // FUNCI�N PARA MOSTRAR SUGERENCIAS GEN�RICAS
    // ========================================
    ...
    function seleccionarValorGenerico(valor, input, container, esNuevo, nombreCampo) {
        ...
        // TODOS los valores se normalizan a MAY�SCULAS
        ...
        if (esNuevo && typeof toastr !== 'undefined') {
            toastr.info(`Nuevo ${nombreCampo} "${valorNormalizado}" ser� agregado al guardar el producto`);
        }
    ...
    // ========================================
    // ACTUALIZAR FUNCI�N PRINCIPAL DE INICIALIZACI�N
    // ========================================
