...
namespace GestionLlantera.Web.Services
{
    public class FacturacionService : IFacturacionService
    {
        ...
        public async Task<(bool success, object? data, string? message, string? details)> ProcesarFacturaConPendientesAsync(object request, string jwtToken = null)
        {
            try
            {
                _logger.LogInformation("⚠️ === PROCESANDO FACTURA CON PRODUCTOS PENDIENTES ===");
                ...
                // Llamar al endpoint del API para procesar con pendientes
                var response = await _httpClient.PostAsync("api/Facturacion/procesar-con-pendientes", content);
                var responseContent = await response.Content.ReadAsStringAsync();
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    return (
                        success: true,
                        data: resultado,
                        message: "Factura procesada con productos pendientes exitosamente",
                        details: null
                    );
                }
                else
                {
                    _logger.LogError("❌ Error del API procesando factura con pendientes: {StatusCode} - {Content}",
                        response.StatusCode, responseContent);
            ...
            {
                _logger.LogError(ex, "❌ Error inesperado procesando factura con pendientes");
                return (
                    success: false,
                    data: null,
                    message: "Error interno al procesar factura con pendientes: " + ex.Message,
                    details: ex.ToString()
                );
            }
        }
