...
function abrirModalServicios() {
    ...
    if (!modalServicios) {
        ...
        if (modalElement) {
            modalServicios = new bootstrap.Modal(modalElement);
        } else {
            console.error('❌ No se encontró el modal de servicios en el DOM');
            mostrarToast('Error', 'No se pudo abrir el modal de servicios', 'danger');
            return;
        }
    }

    // Mostrar modal y cargar servicios
    modalServicios.show();
...
async function cargarServiciosDisponibles() {
    try {
        console.log('🛠️ Cargando servicios disponibles...');
        ...
        const response = await fetch('/Servicios/GetServicios', {
            method: 'GET',
            headers: {
        ...
        console.log('🛠️ Respuesta de servicios:', result);

        if (result.success && result.data) {
            serviciosDisponibles = result.data;
            mostrarServiciosEnModal(serviciosDisponibles);
        } else {
        ...
        $('#serviciosContent').hide();
        $('#serviciosEmpty').show();
        mostrarToast('Error', 'No se pudieron cargar los servicios', 'danger');
    }
}

function mostrarServiciosEnModal(servicios) {
    console.log('🛠️ Mostrando servicios en modal:', servicios);
    ...
    servicios.forEach(servicio => {
        const estadoClase = servicio.activo ? 'bg-success' : 'bg-secondary';
        ...
        html += `
            <tr>
                ...
                <td class="text-end">
                    <strong>₡${formatearMoneda(servicio.precioBase || 0)}</strong>
                </td>
                ...
                <td class="text-center">
                    ${servicio.activo ? `
                        <button type="button" class="btn btn-sm btn-success" onclick="seleccionarServicio(${servicio.servicioId})">
    ...
    $('#serviciosTableBody').html(html);
    $('#serviciosContent').show();
    $('#serviciosEmpty').hide();
}

function seleccionarServicio(servicioId) {
    console.log('🛠️ Seleccionando servicio ID:', servicioId);
    
    const servicio = serviciosDisponibles.find(s => s.servicioId === servicioId);
    if (!servicio) {
        console.error('❌ No se encontró el servicio');
        mostrarToast('Error', 'No se pudo encontrar el servicio seleccionado', 'danger');
        return;
    }

    mostrarModalAgregarServicio(servicio);
}

function mostrarModalAgregarServicio(servicio) {
    console.log('🛠️ Mostrando modal agregar servicio:', servicio);
    ...
    // Inicializar modal agregar servicio si no existe
    if (!modalAgregarServicio) {
        const modalElement = document.getElementById('modalAgregarServicio');
        if (modalElement) {
            modalAgregarServicio = new bootstrap.Modal(modalElement);
        } else {
            console.error('❌ No se encontró el modal de agregar servicio');
            mostrarToast('Error', 'No se pudo abrir el modal de agregar servicio', 'danger');
            return;
        }
    }
    ...
    const detalleHtml = `
        <div class="service-details">
            <h5 class="text-primary">${servicio.nombreServicio}</h5>
            ${servicio.descripcion ? `<p class="text-muted">${servicio.descripcion}</p>` : ''}
            
            <div class="row mb-3">
                <div class="col-6">
                    <strong>Tipo:</strong>
                    <span class="badge bg-info ms-2">${servicio.tipoServicio || 'General'}</span>
                </div>
                <div class="col-6">
                    <strong>Precio Base:</strong>
                    <span class="text-success fs-5 ms-2">₡${formatearMoneda(servicio.precioBase || 0)}</span>
                </div>
            </div>
            
            <div class="alert alert-info">
                <i class="bi bi-info-circle me-2"></i>
                <small>El precio final se calculará según el método de pago seleccionado al finalizar la venta.</small>
            </div>
        </div>
    `;
...
function configurarEventosModalAgregarServicio(servicio) {
    ...
    $('#modalAgregarServicio').off('.servicioModal');
    
    // Botones de cantidad
    $('#btnMenosCantidadServicio').off('click.servicioModal').on('click.servicioModal', function() {
    ...
    $('#btnMasCantidadServicio').off('click.servicioModal').on('click.servicioModal', function() {
        const input = $('#cantidadServicio');
        const valor = parseInt(input.val()) || 1;
        if (valor < 10) { // Límite máximo
            input.val(valor + 1);
        }
    });
    
    // Validar cantidad
    $('#cantidadServicio').off('input.servicioModal').on('input.servicioModal', function() {
    ...
    $('#btnConfirmarAgregarServicio').off('click.servicioModal').on('click.servicioModal', function() {
...
function agregarServicioAVenta(servicio, cantidad = 1, observaciones = '') {
    ...
    // Crear objeto de servicio para el carrito
    const servicioParaVenta = {
        servicioId: servicio.servicioId,
        productoId: null, // Los servicios no tienen productoId
        nombreProducto: servicio.nombreServicio,
        descripcion: servicio.descripcion || '',
        precioUnitario: servicio.precioBase || 0,
        cantidad: cantidad,
        stockDisponible: 999, // Los servicios no tienen stock limitado
        metodoPago: 'efectivo', // Se ajustará al finalizar la venta
        imagenUrl: null,
        esServicio: true,
    ...
    // Verificar si ya existe el mismo servicio
    const servicioExistente = productosEnVenta.find(p => 
        p.esServicio && p.servicioId === servicio.servicioId && p.observaciones === observaciones
    );
    
    if (servicioExistente) {
        servicioExistente.cantidad += cantidad;
    } else {
        productosEnVenta.push(servicioParaVenta);
    }
