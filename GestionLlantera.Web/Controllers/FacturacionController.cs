...
namespace GestionLlantera.Web.Controllers
{
    public class FacturacionController : Controller
    {
        ...
        public FacturacionController(IFacturacionService facturacionService, ILogger<FacturacionController> logger, IHttpClientFactory httpClientFactory)
        {
            _facturacionService = facturacionService;
            _logger = logger;
        ...
        public async Task<IActionResult> ObtenerProductosParaFacturacion()
        {
            try
            {
                ...
                var response = await httpClient.GetAsync("api/Facturacion/productos-venta?soloConStock=true");
                ...
                {
                    return Json(new { success = false, message = "Error al obtener productos de la API" });
                }
            }
            catch (Exception ex)
            {
...
Based on the user message and the changes, I will replace the original FacturacionController with the modified version that includes the HttpClientFactory and the ObtenerProductosParaFacturacion method.