...
// Event listener para el botón "Ver detalles"
        $(document).on('click', '.ver-detalles-btn', function() {
            ...
            // Obtener datos del producto desde la fila
            const producto = {
                productoId: productoId,
                nombreProducto: fila.find('td:eq(0) strong').text(),
                descripcion: fila.find('td:eq(0) small').text().replace('ID: ', ''),
                precioEfectivo: fila.data('precio-efectivo'),
                precioTarjeta: fila.data('precio-tarjeta'),
                stock: fila.data('stock'),
                esLlanta: fila.find('td:eq(2)').text() !== 'N/A',
                medida: fila.find('td:eq(2)').text()
            };

            console.log('🔍 Ver detalles del producto:', producto);
            mostrarModalDetalleProducto(producto, fila);
        });
}
...
function mostrarModalDetalleProducto(producto, fila) {
    console.log('🖼️ Mostrando modal de detalles para:', producto.nombreProducto);

    // Llenar información básica
    $('#detalleProductoId').text(producto.productoId);
    $('#detalleNombreProducto').text(producto.nombreProducto);
    $('#detalleDescripcion').text(producto.descripcion || 'Sin descripción');
    $('#detallePrecioEfectivo').text(`$${parseFloat(producto.precioEfectivo || 0).toLocaleString('es-ES', {minimumFractionDigits: 2})}`);
    $('#detallePrecioTarjeta').text(`$${parseFloat(producto.precioTarjeta || 0).toLocaleString('es-ES', {minimumFractionDigits: 2})}`);
    $('#detalleStock').text(producto.stock || 0);

    // Mostrar medida si es llanta
    if (producto.esLlanta && producto.medida !== 'N/A') {
        $('#contenedorMedidaDetalle').show();
        $('#detalleMedida').text(producto.medida);
    } else {
        $('#contenedorMedidaDetalle').hide();
    }

    // Determinar estado del stock
    const stock = parseInt(producto.stock || 0);
    const $estadoStock = $('#detalleEstadoStock');

    if (stock === 0) {
        $estadoStock.text('Sin Stock').removeClass().addClass('badge bg-danger');
    } else if (stock <= 5) {
        $estadoStock.text('Stock Bajo').removeClass().addClass('badge bg-warning');
    } else {
        $estadoStock.text('Stock Normal').removeClass().addClass('badge bg-success');
    }

    // Cargar imágenes
    cargarImagenesModalDetalle(producto.productoId);

    // Mostrar modal
    $('#modalDetalleProductoInventario').modal('show');
}

function cargarImagenesModalDetalle(productoId) {
    console.log('🖼️ Cargando imágenes para producto ID:', productoId);

    const $contenedorImagenes = $("#contenedorImagenesDetalle");
    const $indicadores = $("#indicadoresDetalle");
    const $btnPrev = $("#btnPrevDetalle");
    const $btnNext = $("#btnNextDetalle");
    ...
    // Hacer petición para obtener imágenes
    $.ajax({
        url: `/Inventario/ObtenerImagenesProducto/${productoId}`,
        type: "GET",
        success: function (imagenes) {
            console.log('🖼️ Imágenes recibidas para detalle:', imagenes);
            procesarImagenesModalDetalle(imagenes);
        },
        error: function (xhr, status, error) {
            console.warn('⚠️ Error al cargar imágenes para detalle:', error);
            // Mostrar imagen por defecto
            procesarImagenesModalDetalle(['/images/no-image.png']);
        }
    });
}

function procesarImagenesModalDetalle(imagenes) {
    console.log('🖼️ Procesando imágenes para modal detalle:', imagenes);
    ...
    // Si no hay imágenes válidas, mostrar imagen por defecto
    const imagenesValidas = Array.isArray(imagenes) && imagenes.length > 0 
        ? imagenes.filter(img => img && img.trim() !== '') 
        : ['/images/no-image.png'];

    console.log('🖼️ Imágenes válidas para procesar:', imagenesValidas);

    // Crear elementos del carrusel
    imagenesValidas.forEach((imagenUrl, index) => {
        const isActive = index === 0 ? 'active' : '';

        // Construir URL completa si es necesaria
        let urlFinal = imagenUrl;
        if (imagenUrl && !imagenUrl.startsWith('http') && !imagenUrl.startsWith('/images/no-image.png')) {
            urlFinal = imagenUrl.startsWith('/') ? 
                `https://localhost:7273${imagenUrl}` : 
                `https://localhost:7273/${imagenUrl}`;
        }
        ...
        `;
        $contenedorImagenes.append(carouselItem);

        // Crear indicador si hay más de una imagen
        if (imagenesValidas.length > 1) {
            const indicador = `
                <button type="button" 
                        data-bs-target="#carouselImagenesDetalle" 
    ...
    if (imagenesValidas.length > 1) {
        ...
        console.log('🖼️ Carrusel activado con', imagenesValidas.length, 'imágenes');
    } else {
        $btnPrev.hide();
        $btnNext.hide();
        $indicadores.hide();

        console.log('🖼️ Solo una imagen, carrusel desactivado');
    }

    // Inicializar el carrusel de Bootstrap
    const carouselElement = document.getElementById('carouselImagenesDetalle');
    if (carouselElement && imagenesValidas.length > 1) {
