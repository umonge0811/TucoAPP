...
namespace GestionLlantera.Web.Controllers
{
    ...
    public class FacturacionController : Controller
    {
        ...
        public async Task<IActionResult> VerificarVencimiento()
        {
            try
            {
                _logger.LogInformation("📅 === VERIFICACIÓN MANUAL DE VENCIMIENTO DE PROFORMAS ===");
                ...
                var response = await client.PostAsync($"{_configuration["ApiSettings:BaseUrl"]}/api/Facturacion/verificar-vencimiento-proformas?esVerificacionAutomatica=false", 
                    new StringContent("{}", System.Text.Encoding.UTF8, "application/json"));
        ...
        [HttpPost]
        public async Task<IActionResult> MarcarProformaComoFacturada(int proformaId, [FromBody] object request)
        {
            try
            {
                ...
                var resultado = await _facturacionService.MarcarProformaComoFacturadaAsync(proformaId, token);
            ...
            catch (Exception ex)
            {
                ...
                return Json(new { success = false, message = "Error interno al marcar proforma como facturada" });
            }
        }
    }
