...
namespace GestionLlantera.Web.Controllers
{
    ...
    public class TomaInventarioController : Controller
    {
        ...
        /// <summary>
        /// Obtiene las discrepancias reales de un inventario programado
        /// </summary>
        [HttpGet]
        public async Task<IActionResult> ObtenerDiscrepancias(int inventarioId)
        {
            try
            {
                ...
                // ✅ LLAMAR AL SERVICIO PARA OBTENER DISCREPANCIAS
                var discrepancias = await _tomaInventarioService.ObtenerDiscrepanciasAsync(inventarioId, token);

                if (discrepancias == null)
                {
                    _logger.LogWarning("⚠️ No se pudieron obtener las discrepancias del inventario {InventarioId}", inventarioId);
                    return Json(new { 
                        success = false, 
                        message = "No se pudieron obtener las discrepancias del inventario" 
                    });
                }

                _logger.LogInformation("✅ Discrepancias obtenidas: {Count}", discrepancias.Count);

                return Json(new
                {
                    success = true,
                    data = discrepancias,
                    count = discrepancias.Count,
                    message = discrepancias.Count > 0 
                        ? $"Se encontraron {discrepancias.Count} producto(s) con discrepancias"
                        : "No se encontraron discrepancias en este inventario"
                });
            }
            catch (Exception ex)
            {
                ...
                {
                    success = false,
                    message = "Error interno al obtener las discrepancias",
                    error = ex.Message
                });
            }
        }
