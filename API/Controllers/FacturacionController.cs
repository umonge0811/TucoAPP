...
namespace API.Controllers
{
    ...
    public class FacturacionController : ControllerBase
    {
        ...
        [HttpGet("buscar-pendiente-por-codigo")]
        [Authorize]
        public async Task<IActionResult> BuscarPendientePorCodigo([FromQuery] string codigo)
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(codigo))
                {
                    return BadRequest(new { 
                        success = false, 
                        message = "Debe proporcionar un código de seguimiento válido" 
                    });
                }
                ...
                var pendiente = await _context.PendientesEntrega
                    .Include(p => p.Producto)
                        .ThenInclude(pr => pr.Llanta)
                    .Include(p => p.Factura)
                    .Where(p => p.CodigoSeguimiento == codigo && p.Estado == "Pendiente")
                    .Select(p => new
                    {
                        id = p.Id,
                        facturaId = p.FacturaId,
                        ...
                        cantidadSolicitada = p.CantidadSolicitada,
                        cantidadPendiente = p.CantidadPendiente,
                        stockDisponible = (int)(p.Producto.CantidadEnInventario ?? 0),
                        fechaCreacion = p.FechaCreacion,
                        estado = p.Estado,
                        observaciones = p.Observaciones,
                ...
                if (pendiente == null)
                {
                    _logger.LogInformation("🔍 No se encontró pendiente con código: {Codigo}", codigo);
                    return NotFound(new { 
                        success = false, 
                        message = $"No se encontró ningún producto pendiente con el código '{codigo}'" 
                    });
                }

                _logger.LogInformation("✅ Pendiente encontrado: {Producto} para factura {NumeroFactura}", 
                    pendiente.nombreProducto, pendiente.numeroFactura);

                return Ok(new
                {
                    success = true,
                    pendiente = pendiente,
                    message = "Producto pendiente encontrado exitosamente"
                });
            }
            catch (Exception ex)
            {
                ...
                return StatusCode(500, new { 
                    success = false, 
                    message = "Error interno al buscar el pendiente" 
                });
            }
        }

        [HttpPost("entregar-pendiente")]
        [Authorize]
        public async Task<IActionResult> EntregarPendiente([FromBody] EntregarPendienteApiRequest request)
        {
            var validacionPermiso = await this.ValidarPermisoAsync(_permisosService, "Entrega Pendientes",
                "Solo usuarios con permiso para entrega de pendientes pueden realizar esta acción");
            if (validacionPermiso != null) return validacionPermiso;
            ...
            try
            {
                ...
                var pendiente = await _context.PendientesEntrega
                    .Include(p => p.Producto)
                    .Include(p => p.Factura)
                    .FirstOrDefaultAsync(p => p.Id == request.PendienteId);

                if (pendiente == null)
                {
                    return NotFound(new { 
                        success = false, 
                        message = "Pendiente de entrega no encontrado" 
                    });
                }
                ...
                if (pendiente.Estado != "Pendiente")
                {
                    return BadRequest(new { 
                        success = false, 
                        message = "Este pendiente ya fue procesado anteriormente" 
                    });
                }
                ...
                if (stockDisponible < cantidadAEntregar)
                {
                    return BadRequest(new { 
                        success = false,
                        message = $"Stock insuficiente para completar la entrega. Disponible: {stockDisponible}, Requerido: {cantidadAEntregar}" 
                    });
                }
                ...
                var usuarioNombre = User.FindFirst(System.Security.Claims.ClaimTypes.Name)?.Value ?? "Usuario";

                // Actualizar pendiente como entregado
                pendiente.Estado = "Entregado";
                pendiente.FechaEntrega = DateTime.Now;
                pendiente.UsuarioEntrega = int.TryParse(usuarioId, out int uid) ? uid : 1;
                
                var observacionesEntrega = request.ObservacionesEntrega ?? 
                    $"Entregado en punto de venta por {usuarioNombre}";
                
                pendiente.Observaciones = (pendiente.Observaciones ?? "") + 
                    $" | ENTREGADO: {cantidadAEntregar} unidades el {DateTime.Now:dd/MM/yyyy HH:mm} por {usuarioNombre}. {observacionesEntrega}";

                await _context.SaveChangesAsync();
                await transaction.CommitAsync();

                _logger.LogInformation("✅ Pendiente entregado exitosamente: {Producto} - {Cantidad} unidades", 
                    pendiente.Producto.NombreProducto, cantidadAEntregar);

                return Ok(new
                {
                    success = true,
                    message = $"Producto '{pendiente.Producto.NombreProducto}' entregado exitosamente",
                    cantidadEntregada = cantidadAEntregar,
                    stockRestante = pendiente.Producto.CantidadEnInventario,
            ...
            {
                await transaction.RollbackAsync();
                _logger.LogError(ex, "❌ Error al entregar pendiente: {PendienteId}", request.PendienteId);
                return StatusCode(500, new { 
                    success = false, 
    ...
    public class EntregarPendienteApiRequest
    {
